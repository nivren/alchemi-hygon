...
与用户用中文沟通。实现、测试、数值结果和未验证假设必须分别报告。恢复实质开发、修改代码或配置、判断当前支持状态时，按任务需要读取 `docs/PROJECT_HANDOFF.md`、`docs/STATUS.md` 的相关章节；纯问答、局部只读审查、拼写或格式小改不强制通读。涉及首次部署时读 `docs/START_HERE.md`。需要背景细节时读 `docs/海光DCU移植开发路线与工程规范.md`。
...
仅当用户明确要求首次部署、重新初始化或“按文档开始”时，读取 `docs/START_HERE.md`，结合
`docs/STATUS.md` 只执行尚未完成的步骤；不得重复初始化、覆盖已有工作或把普通开发任务扩大为
全量环境盘点。首轮完成标准和后续 G1/G2 验收边界以 `START_HERE`、`PROJECT_HANDOFF` 的相关
章节为准。
...
性能必须实测，区分冷启动/JIT、预热后的执行、端到端成本和通信。共享服务器运行 HCU 任务前先只读确认资源；使用临时夹具的本地 CPU 测试，以及在已分配或确认空闲设备上的限时单卡 smoke，可直接执行而无需逐步请求确认。没有可用资源时跳过并报告，不原地等待。探针限时限量，不升级系统驱动/DTK，不杀他人任务，不自行占满所有 GPU。长作业、多卡占用、系统级安装和外部发布按用户授权执行。

只有任务改变当前支持状态、验证证据或阻塞项时才更新 STATUS；影响接口、后端或支持范围时更新特性契约和必要的 ADR；同步上游时更新 UPSTREAM。纯拼写、格式或不改变行为的局部重构不强制联动这些文档。一次修改聚焦一个可验证目标并保留可审查 diff；在用户已授权的目标内完成实现及相关验证，不因得到首个实现就自动暂停，只有重大选择或用户明确要求的里程碑才等待确认。共享基线使用 `develop`/阶段分支，个人分支按 `<开发者>/<类型>-<主题>` 命名，不自动推送。
...
会修改仓库的任务开始时按以下顺序执行；纯问答和局部只读审查只读取直接相关内容：

1. 运行 `git status --short --branch`、查看最近提交和目标文件 diff。工作树已有改动时，
   不使用 `reset --hard`、`checkout --`、全库格式化或覆盖式复制；先避开无关文件，无法避开
   时向用户报告。
2. 恢复实质开发或需要判断当前状态时，按标题、搜索结果和任务范围读取
   `docs/PROJECT_HANDOFF.md`、`docs/STATUS.md` 的相关章节；涉及环境时再读部署指南和
   `docs/ENVIRONMENT.md`。
3. 从锁定 SHA 对应的 `packages/` 源码、测试和 docstring 建立事实；`external/` 只读，不能
   直接修改，也不能把探索环境结果写成当前产品证据。
4. 涉及后端或数值语义时，先写适用的算子/功能契约和最小 reference 测试，再实现后端、接
   framework；仅在状态、证据、接口或决策实际变化时同步相应文档。
...
新后端或改变设备路径、数值语义的算子至少要有：上游来源和符号、输入输出/shape/dtype/layout/单位、PBC 与
full/half neighbor 约定、空输入/容量/错误、alias/mutation/stream/确定性、梯度等级、CPU
reference 和相关回归测试。代码实际触及 HCU 路径且资源可用时运行限时 HCU smoke，并按需补
探针报告和兼容性条目；资源不可用时明确记录未验证范围，除非当前验收门要求 HCU，否则不阻塞
其余工作。普通上层功能按实际影响选择适用项。不能只补一个“能跑”的示例。

Git 协作默认从 `develop` 或当前阶段分支创建个人 `<开发者>/<类型>-<主题>` 分支，使用短而单一目的的提交和 `git commit -s`。正常工作按
路径显式 `git add`，不使用 `git add -A` 吞入他人改动；导入、实现、测试、文档尽量分开提交。
提交前运行 `git diff --check` 和相关 CPU 测试；修改 HCU 路径且有可用资源时再运行限时 HCU
探针，无资源则报告未验证边界而不等待。交接中分别报告实现、测试、数值结果与未验证假设。
不要自动推送、不要改写共享提交历史；同步上游前先建立单独
