> 这是历史交接日志，不是当前交接入口。当前稳定边界请阅读 [`../PROJECT_HANDOFF.md`](../PROJECT_HANDOFF.md)。

# 项目背景、兼容性契约与实施路线

这份文档汇总此前讨论中与开发有关的需求和约定。演示稿排版及代码展示要求不属于产品需求。
当前已完成的移植、实测证据和暂停点以本节、`docs/STATUS.md` 当前摘要和
`docs/BACKEND_CAPABILITY_MATRIX.md` 为准；后面的阶段/Point 条目是历史交接日志，早期章节
中的“未完成”描述不覆盖后续状态记录。

## 当前交接摘要（2026-09-20）

- 当前代码基线为 `develop`，阶段二收口提交为 `9f53f80`，文档收口提交为 `9e295ea`；两者
  已推送到 `local-origin/develop` 和 `github-origin/develop`。
- `TORCH-NEIGHBOR-PBC-CELL` 已完成阶段一 Torch reference 和阶段二 native HIP 窄 capability
  的实现、测试、HCU gate 与文档收口。显式 HIP 范围是 periodic/fixed-cell/Batch/full-list/
  MATRIX、FP32/FP64、`skin=0`，包含有限的自动容量增长 contract。
- 该结果不等同完整上游邻居支持；skin/rebuild、target/pair、pair-centric/sorted query、
  变胞、native geometry backward、compile/opcheck、DomainParallel、`auto` 和生产默认性能
  仍未完成。Torch geometry 继续是训练/梯度路径，native geometry 只是 forward-only 候选。
- 当前计划见 `docs/PARALLEL_DEVELOPMENT_PLAN.md`：Langevin 剩余 reference 生命周期、固定晶胞
  NHC、FIRE2 变胞 stress→cell-force、固定晶胞 ASE-compatible BFGS。未得到用户确认前不自动
  启动新分支。
- 本机后续单卡 HCU 验证统一使用 `HIP_VISIBLE_DEVICES=4`；历史日志中的 HCU 0 是真实历史
  证据，不改写。
