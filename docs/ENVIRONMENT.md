...
原始探针输出放 artifacts/g0 和 artifacts/g1，摘要放 reports。当前已确认用户缓存中的 `MACE-OFF23_small.model` 可在探索环境 CPU/HCU 运行；项目 `.venv` 仍未安装 MACE/e3nn/ASE。设备型号、安装版本和编译成功不等于算子或产品支持；两个 perf_46 结构的 HCU wrapper batching 仍因 180 秒超时未验证。
