...
| P06-wrapper | `source scripts/activate_hygon_env.sh exploration` 后执行 `python probes/mace_wrapper_reference.py --device cpu --checkpoint /home/wangleping/.cache/mace/MACE-OFF23_small.model [--cif <file1> <file2> ...]`；HCU 版增加 `HIP_VISIBLE_DEVICES=<id> OMP_NUM_THREADS=1 timeout 180` | H₂O 单/双体系和单个 `/data/csp_data/perf_46` 结构的 wrapper CPU/HCU 已通过；两个 `perf_46` CIF 的 CPU batching 已通过，`batch_ptr`/逐体系 energy/无跨体系边/逐体系总力均检查；同一批次 HCU 超时退出 `124`，问题收窄为多体系 HCU Batch，仍不算通过 |
