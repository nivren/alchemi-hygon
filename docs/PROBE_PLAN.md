...
| P02 编译 | `hipcc --offload-arch=gfx928 probes/hip_probe.cpp -o artifacts/g0/hip_probe` | 编译退出 0；gfx928 是显式候选，需运行时核对 |
...
P04 自定义算子的 fake/meta、opcheck、gradcheck/gradgradcheck，P05 PBC/LJ/NVE 闭环、P07 两卡 LJ ownership/pair 守恒尚待 G1 后端实现；当前 probe 不掩盖这些覆盖缺口。所有 GPU probe 在这轮均未运行。无 NVIDIA 对拍 fixture。
