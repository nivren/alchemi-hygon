...
MACE 审计和 checkpoint 证据见 `reports/g1-mace-dependency-audit.md`、`reports/g1-mace-wrapper-batch.md`。探索环境和项目 `.venv` 均已完成 MACE 最小依赖安装；项目环境用北外镜像解析到 `matscipy==1.1.1`，未替换海光 Torch/Triton。PhysicsNeMo 未安装，单进程 wrapper 通过可选导入路径验证，域并行/profiling 仍单独登记。探针输出 checkpoint/CIF SHA256、逐体系 batch 边界、energy/force 和 direct model 参数梯度；未知 checkpoint 不下载，不以 mock 代替真实 MLIP。
